...
auth.post('/login', zValidator('json', loginSchema), async (c) => {
  ...
  try {
    ...
    // Verify password
    const valid = await bcrypt.compare(password, user.hashed_password as string);
    if (!valid) {
      return c.json({ error: 'Invalid credentials' }, 401);
    }
    ...
    // Create token
    const token = await createToken(user.id as number, user.username as string, c.env.JWT_SECRET);
  ...
  try {
    ...
    const result = await c.env.DB.prepare(
      `INSERT INTO users (username, email, hashed_password, full_name, is_active, is_superuser)
       VALUES (?, ?, ?, ?, 1, 0)`
    ).bind(username, email, hashedPassword, full_name || null).run();
