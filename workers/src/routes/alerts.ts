...
alerts.post('/:id/acknowledge', zValidator('json', acknowledgeSchema), async (c) => {
  const id = c.req.param('id');
    ...
    await c.env.DB.prepare(
      `UPDATE alerts 
       SET status = 'acknowledged', 
           ...
           notes = ?
       WHERE id = ?`
    ).bind(user.username, notes || null, id).run();
...
// Resolve alert
alerts.post('/:id/resolve', zValidator('json', acknowledgeSchema), async (c) => {
  const id = c.req.param('id');
  const { notes } = c.req.valid('json');
  const user = c.get('user');
  ...
  try {
    await c.env.DB.prepare(
      `UPDATE alerts 
       ...
       WHERE id = ?`
    ).bind(user.username, notes || '', id).run();
